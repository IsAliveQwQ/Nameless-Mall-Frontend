...
export default function PromotionDetailPage({ params }: { params: { id: string } }) {
    ...
    if (isFlashSale && flashSale) {
        // Use cleaned product name for deduplication count as a safer fallback
        const uniqueProductCount = new Set(flashSale.products.map(p =>
            p.name.replace(/\s+SKU-[\d-]+.*$/i, '').trim()
        )).size;
        hero = {
        ...
        header = {
            ...
            description: `Nameless Mall 限時特賣火熱進行中！精選商品限量釋出，立即搶購。錯過不再。此場次包含 ${uniqueProductCount} 件精選商品。`,
            code: 'FLASH_SALE',
