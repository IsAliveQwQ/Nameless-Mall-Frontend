...
export const FlashSaleProductGrid: React.FC<{ products: FlashSaleProductVO[] }> = ({ products }) => {
    // Deduplicate products by cleaned name, keeping the one with the lowest flash price
    const uniqueProductsMap = new Map<string, FlashSaleProductVO>();
    products.forEach(p => {
        const cleanName = p.name.replace(/\s+SKU-[\d-]+.*$/i, '').trim();
        const existing = uniqueProductsMap.get(cleanName);
        if (!existing || (p.flashPrice || 0) < (existing.flashPrice || 0)) {
            uniqueProductsMap.set(cleanName, p);
        }
    });
        ...
        <div className="space-y-16">
                            ...
                            <div className="py-5 px-1 flex flex-col gap-2">
                                <div className="flex flex-col">
                                    ...
                                    <h3 className="text-[15px] text-zinc-900 font-bold tracking-tight mb-2 group-hover:text-zinc-600 transition-colors line-clamp-1">
                                        {product.name.replace(/\s+SKU-[\d-]+.*$/i, '').trim()}
                                    </h3>
                                </div>
            ...
            {uniqueProducts.length > 15 && (
                <div className="flex justify-center border-t border-zinc-100 pt-12">
                    ...
                    >
                        Explore Complete Collection ({uniqueProducts.length})
